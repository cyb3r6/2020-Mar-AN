...
{
    public Lever forwardBackwardLever, rightleftLever, upDownLever, dialLever;
    ...
    void Update()
    {
        ...
        // rotate the cube
        if (Mathf.Abs(dialLever.NormalizedJointAngle()) > 0.05f)
        {
            transform.Rotate(Vector3.up, dialLever.NormalizedJointAngle() * Time.deltaTime * rotateSpeed);
        }
    }
